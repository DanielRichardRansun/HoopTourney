...
<div class="flex items-center gap-3 mb-8">
    <div class="size-12 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center">
        <span class="material-symbols-outlined text-emerald-500 text-2xl">add_circle</span>
    </div>
    <div>
        <h1 class="text-2xl md:text-3xl font-black text-white uppercase tracking-tight">Insert Hasil Pertandingan</h1>
...
<form action="{{ route('matchResults.store', ['id_tournament' => $tournament->id, 'id_schedule' => $schedule->id]) }}" method="POST">
    ...
    <div class="glass-panel rounded-2xl border border-[#393028] p-6 mb-6 shadow-xl">
        <div class="flex items-center gap-2 mb-5">
            ...
            <h3 class="text-white font-black uppercase tracking-wider text-sm">Hasil Pertandingan per Kuarter</h3>
        </div>
        <div id="quarter-scores-container" class="space-y-4">
            <div class="quarter-input-group glass-panel rounded-xl p-4 border border-[#393028]" data-quarter-number="1">
                <label class="block text-primary text-xs font-bold uppercase tracking-widest mb-3">Kuarter 1</label>
                <div class="flex items-center gap-3">
                    <div class="flex-1">
                        <label class="text-slate-500 text-[10px] uppercase font-bold tracking-wider">{{ $team1->name }}</label>
                        <input type="number" name="quarter_scores[1][team1_score]" class="w-full bg-[#221914] border border-[#393028] text-white rounded-lg px-3 py-2.5 outline-none focus:border-primary transition-colors text-center font-bold" required min="0" placeholder="0">
                    </div>
                    ...
                    <div class="flex-1">
                        ...
                        <input type="number" name="quarter_scores[1][team2_score]" class="w-full bg-[#221914] border border-[#393028] text-white rounded-lg px-3 py-2.5 outline-none focus:border-primary transition-colors text-center font-bold" required min="0" placeholder="0">
                    </div>
                </div>
            </div>
        </div>
        ...
        @foreach($players1 as $player)
            <div class="glass-panel rounded-xl p-4 mb-4 border border-[#393028]">
                ...
                <div id="player-stats-{{ $player->id }}-container">
                    <div class="player-stats bg-[#221914] rounded-lg p-3 mb-2" data-quarter-number="1">
                        <label class="block text-primary text-[10px] font-bold uppercase tracking-widest mb-2">Kuarter 1</label>
                        <div class="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-12 gap-2">
                            @foreach(['point'=>'PTS','fgm'=>'FGM','fga'=>'FGA','fta'=>'FTA','ftm'=>'FTM','orb'=>'ORB','drb'=>'DRB','stl'=>'STL','ast'=>'AST','blk'=>'BLK','pf'=>'PF','to'=>'TO'] as $key => $label)
                                <div class="text-center">
                                    ...
                                    <input type="number" name="player_stats[{{ $player->id }}][1][{{ $key }}]" min="0" class="w-full bg-[#181411] border border-[#393028] text-white rounded-md px-1 py-1.5 outline-none focus:border-primary text-center text-xs font-semibold">
                                </div>
                            @endforeach
                        </div>
                    </div>
